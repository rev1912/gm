      ...
      !function(e, t) {
        ...
        return function() {
          var __webpack_modules__ = { 3312: function(e, t, n) {
            ...
            function R() {
              O && (P.text("".concat(window.innerWidth, "px \xD7 ").concat(window.innerHeight, "px")), L ? clearTimeout(L) : document.documentElement.appendChild(F), L = setTimeout(function() {
                P.remove(), L = null;
              }, 1e3));
            }
              ...
              return r(t2, e2), t2.prototype.setContainer = function(e3) {
                ...
                return e3.showRulers && this.drawAxis(this.context, r2, i2), e3.paths.length && (e3.showExtensionLines && function(e4, t4, n3, o3, r3, i3, a2, s2) {
                  ...
                  var u2 = (0, c.createChild)(e4, "div"), d2 = (0, c.createChild)(u2, "div", "tooltip-content"), h2 = function(e5, t5) {
                    ...
                    (0, c.createChild)(f3, "span", "material-node-width").textContent = String(Math.round(100 * e5.nodeWidth) / 100), (0, c.createTextChild)(f3, "\xD7"), (0, c.createChild)(f3, "span", "material-node-height").textContent = String(Math.round(100 * e5.nodeHeight) / 100);
                    var p3, v2 = e5.style || {};
...
/*# sourceMappingURL=luna-dom-highlighter.css.map*/`;
  ...
  function setAttributeNS(node, namespace, name2, value) {
    if (isHydrating(node)) return;
    if (value == null) node.removeAttributeNS(namespace, name2);
    else node.setAttributeNS(namespace, name2, value);
  }
    ...
    if (current2.length) {
      ...
      for (let i = current2.length - 1; i >= 0; i--) {
        ...
        if (node !== el) {
          ...
          if (!inserted && !i)
            isParent ? parent2.replaceChild(node, el) : parent2.insertBefore(node, marker2);
          else isParent && el.remove();
        } else inserted = true;
      }
    } else parent2.insertBefore(node, marker2);
    return [node];
  }
  ...
  function parseTag(tag) {
    ...
    for (const match2 of tag.matchAll(reg)) {
      if ((match2[1] || match2[2]).startsWith("use:")) {
        res.attrs.push({
          type: "directive",
          name: match2[1] || match2[2],
          value: match2[4] || match2[5] || ""
        });
      } else {
        res.attrs.push({
          type: "attr",
  ...
  function pushTextNode(list2, html2, start2) {
    const end = html2.indexOf("<", start2);
    const content = html2.slice(start2, end === -1 ? void 0 : end);
    if (!/^\s*$/.test(content)) {
      list2.push({
        type: "text",
        content
  ...
  function pushCommentNode(list2, tag) {
  ...
  function createHTML(r, { delegateEvents: delegateEvents2 = true, functionBuilder = (...args) => new Function(...args) } = {}) {
    ...
    function parseKeyValue(node, tag, name2, value, isSVG, isCE, options) {
      ...
      if ((parts = name2.split(":")) && parts[1] && reservedNameSpaces.has(parts[0])) {
        name2 = parts[1];
        namespace = parts[0];
      }
      ...
      if (name2 === "style") {
        ...
        options.exprs.push(
          `${tag}.${r.getPropAlias(name2, node.name.toUpperCase()) || name2} = ${expr}`
        );
      } else {
        const ns = isSVG && name2.indexOf(":") > -1 && r.SVGNamespace[name2.split(":")[0]];
        if (ns) options.exprs.push(`r.setAttributeNS(${tag},"${ns}","${name2}",${expr})`);
        else options.exprs.push(`r.setAttribute(${tag},"${r.Aliases[name2] || name2}",${expr})`);
      }
    }
    function parseAttribute(node, tag, name2, value, isSVG, isCE, options) {
      if (name2.slice(0, 2) === "on") {
        if (!name2.includes(":")) {
          const lc = name2.slice(2).toLowerCase();
          ...
          let capture = name2.startsWith("oncapture:");
          options.exprs.push(
            `${tag}.addEventListener("${name2.slice(capture ? 10 : 3)}",exprs[${options.counter++}]${capture ? ",true" : ""})`
          );
        }
      } else if (name2 === "ref") {
...
${childOptions.exprs.join(
    ...
    function processComponent(node, options) {
      let props = [];
      const keys2 = Object.keys(node.attrs), propGroups = [props], componentIdentifier = options.counter++;
      for (let i = 0; i < keys2.length; i++) {
        ...
        if (type === "attr") {
          ...
          parseDirective(name2, value, tag2, options);
        }
      }
        ...
        if (node.attrs.some((e) => e.name === "###")) {
          ...
          for (let i = 0; i < node.attrs.length; i++) {
            ...
            if (type === "attr") {
              ...
              parseDirective(name2, value, tag, options);
            }
          }
          node.attrs = newAttrs;
          if (current2.length) {
          ...
          for (let i = 0; i < node.attrs.length; i++) {
            ...
            if (type === "directive") {
              ...
              if (value.includes("###")) {
                node.attrs.splice(i, 1);
                i--;
                parseAttribute(node, tag, name2, value, isSVG, isCE, options);
              }
            }
          }
        }
...
`)
  ...
  var polar2Cart = (cx, cy, r, a) => ({
    x: cx + r * Math.cos(a),
    y: cy + r * Math.sin(a)
  });
  var statsChart = () => {
    let svg;
    let cx = 100, cy = 100, r = 100, sides = 8;
    let step = 2 * Math.PI / sides;
    let rotation = Math.PI / 8;
    const values = [1, 0.75, 0.4, 0.9, 0.6, 0.5, 0.85, 0.3];
    let baseP = [], dataP = [], vertices = [];
    createEffect(() => {
      range(0, sides).forEach((i) => {
        let a = i * step - Math.PI / 2 + rotation;
        let base = polar2Cart(cx, cy, r, a);
        baseP.push(`${base.x},${base.y}`);
        vertices.push(base);
        let data = polar2Cart(cx, cy, r * values[i], a);
        dataP.push(`${data.x},${data.y}`);
        let dp = document.createElementNS("http://www.w3.org/2000/svg", "circle");
        dp.setAttribute("class", "data-point");
        dp.setAttribute("r", "4");
        dp.setAttribute("cx", data.x);
        dp.setAttribute("cy", data.y);
        svg.appendChild(dp);
      });
      let basePoly = document.createElementNS("http://www.w3.org/2000/svg", "polygon");
      basePoly.setAttribute("points", baseP.join(" "));
      basePoly.classList.add("base");
      svg.appendChild(basePoly);
      let dataPoly = document.createElementNS("http://www.w3.org/2000/svg", "polygon");
      dataPoly.setAttribute("points", dataP.join(" "));
      dataPoly.classList.add("data");
      svg.appendChild(dataPoly);
      let inner = [0.25, 0.5, 0.75];
      inner.forEach((scale) => {
        let innerP = [];
        for (let i = 0; i < sides; i++) {
          let a = i * step - Math.PI / 2 + rotation;
          let point = polar2Cart(cx, cy, r * scale, a);
          innerP.push(`${point.x},${point.y}`);
        }
        let ring = document.createElementNS("http://www.w3.org/2000/svg", "polygon");
        ring.setAttribute("points", innerP.join(" "));
      ...
      clip.setAttribute("id", "octagon-clip");
      let clipPoly = document.createElementNS("http://www.w3.org/2000/svg", "polygon");
      clipPoly.setAttribute("points", baseP.join(" "));
      ...
      bgRect.setAttribute("fill", "#111");
      bgRect.setAttribute("clip-path", "url(#octagon-clip)");
  ...
  var range2 = (start2, end) => Array.from({ length: end - start2 }, (_, i) => start2 + i);
  var overlay;
  var wrestlerBook = () => {
    let book;
    let open = false;
    createEffect(() => {
      overlay.querySelector(".paper-page").addEventListener("click", () => {
        book.style.transform = "translateX(0%)";
        overlay.querySelector(".secret p").style.background = "#333";
        open = false;
      });
    });
    let flipPage = (e, id) => {
      if (book.style.transform == "translateX(100%)") {
        book.style.transform = "translateX(0%)";
        overlay.querySelector(".secret p").style.background = "#333";
        open = false;
        return;
      }
      let lab = overlay.querySelectorAll(".bm-label");
      lab.forEach((l) => l.style.background = "#333");
      e.target.style.background = "#111";
      let pages = book.querySelectorAll(".paper-page:not(.paper-page:first-child)");
      pages.forEach((page2, i) => {
        if (i == id) page2.style.display = "flex";
        i < id ? page2.classList.add("flip") : page2.classList.remove("flip");
      });
    };
    let secret = (e) => {
      if (open == false) {
      ...
      if (book.style.transform == "" || book.style.transform == "translateX(0%)") {
        book.style.transform = "translateX(100%)";
      } else book.style.transform = "translateX(0%)";
    };
    return html`
    <div ref=${(e) => overlay = e} class=overlay>
      <div class=board-h>
        <div class=bookmark onClick=${(e) => flipPage(e, 0)}><div class=bm-label>Bio</div>
        </div>
        <div class=bookmark onClick=${(e) => flipPage(e, 1)}><div class=bm-label>Status</div>
        </div>
        <div class=bookmark onClick=${(e) => flipPage(e, 2)}><div class=bm-label>Records</div>
        </div>
        <div class=bookmark onClick=${(e) => flipPage(e, 3)}><div class=bm-label>Skill</div>
        </div>
      </div>
    <div class=board-container>
      <div class=secret onClick=${secret}>
        <p>Attr</p>
      </div>
      <div class=board-body>
        <div class=paper-page>
          <div class=square-clip></div>
          <div class=paper></div>
          <div class=redline></div>
          <div class=paper-content>
            <div class=content-hd>WRATH</div>
            
            <div class=cover>
              <div class=paperclip></div>
              <img id=cover-img src=dummy.png>
            </div>
            <div class=stats-container>
              <div class=stats1>STR<br>10.0</div>
              <div class=stats2>SPD<br>10.0</div>
              <div class=stats3>SUB<br>10.0</div>
              <div class=stats4>CHA<br>10.0</div>
              <div class=stats5>DUR<br>10.0</div>
              <div class=stats6>HRD<br>10.0</div>
              <div class=stats7>TEC<br>10.0</div>
              <div class=stats8>STA<br>10.0</div>
              ${statsChart()}
            </div>
          </div>
        </div>
        <div ref=${(e) => book = e} class=book>
          ${() => range2(0, 4).map((n) => {
      return html`
              <div id="page${n}" class=paper-page>
                <div class=square-clip></div>
                <div class=paper></div>
                <div class=redline></div>
                <div class=paper-content>
            ...
            <div class=text>Weight Class</div>
            <div class=text>Speciality Match Type</div>
            <div class=text>Injury Frequency</div>
      ...
      <button class=closebtn onClick=${() => overlay.classList.remove("active")}>X</button>
    </div>
  ...
  var TableBody = (store2) => {
    return () => store2.roster.map((w2, i) => {
      ...
      <div class=row>
        ...
        <div class=cell-data><div class=cell-bg>${96}</div></div>
        <div class=cell-data>
          <div class=cell-bg>
            <div class=fat-bar>
              <div class=fat-progress style=${fat}>
  ...
  var getCenterItem = () => {
    ...
    let centerY = trackRect.top + track.clientHeight / 2;
    let closest = null;
    let closestDist = Infinity;
    for (const elem of list.children) {
      let rect = elem.getBoundingClientRect();
      let elemCenter = rect.top + rect.height / 2;
      let distance = Math.abs(centerY - elemCenter);
  ...
  var move = (event) => {
    ...
    if (Math.abs(deltaY) >= itemH) {
      deltaY > 0 ? track.scrollTop += itemH : track.scrollTop -= itemH;
      track.scrollTop = Math.round(track.scrollTop / itemH) * itemH;
      startY = curY;
    }
  };
  var newItems = () => items.map((text) => {
    let div = document.createElement("div");
    div.className = "roster";
    div.textContent = text;
    return div;
...
<div class="svg-net2">
  ...
  var range8 = (start2, end) => Array.from({ length: end - start2 }, (_, i) => start2 + i);
  var smackdown = () => {
    let data = [
      { name: "Batista", img: "batista" },
      { name: "Booker T", img: "booker_t" },
      { name: "Chris Benoit", img: "benoit" },
      { name: "Eddie Guerrero", img: "eddie" },
      { name: "John Bradshaw Layfield", img: "jbl" },
      { name: "Muhammad Hassan", img: "hassan" },
      { name: "Randy Orton", img: "randy" },
      { name: "Rey Mysterio", img: "rey" },
      { name: "Undertaker", img: "undertaker" }
    ];
